...
with open('../../Downloads/ml-latest/ml-latest/movies.csv', mode='r') as file:
    ...
    for lines in csvFile:
        ...
        movie_num = lines[0]
        movie = lines[1]
        pattern = re.compile('(?P<title1>.*?)(?:(, )(?P<title2>.*))? (?P<year>\([0-9]*\))\"?')
        if re.match(pattern, movie):
            match = re.match(pattern, movie)
            if match.groupdict()["title2"]:
                title = match.groupdict()["title2"] + " " + match.groupdict()["title1"]
            else:
                title = match.groupdict()["title1"]

            year = match.groupdict()["year"]

            results = ia.search_movie(title + " " + year)
            if len(results) == 0:
                continue
            result = results[0]
